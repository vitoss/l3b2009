...
//Operatory
//W razie porażki zwraca 0 i wyswietla odpowiedni komunikat
...
Matrix & Matrix::operator = (const Matrix &mat) {
	if( *this == mat ) return *this; // w celu zapobiegnięciu zbędnej pracy A = A

	if( ((this->mw) != (mat.mw)) || (this->mk != mat.mk) ) {
		cout << "Wymiary macierzy się nie zgadzają" <<endl;
		return *this;
	}
	for( unsigned int i = 0; i<mw; i++ ) {
		for( unsigned int k = 0; k<mk; k++ ) {
...
Matrix &Matrix::operator -= (const Matrix &mat) {
	...
	//Wersja smart, ale musimy skopiować zawartość mat (które jest const).
...
istream& operator>> (istream& is, Matrix &mat) {
	cout << "Rozpoczęto procedure wcztywania macierzy z klawiatury"<<endl;
	...
	for( unsigned int i = 0; i<mat.mw; i++ ) {
		cout << "Proszę podać " << mat.mk << " wartości typu double oddzielonych spacjami. ("<<i<<" wiersz)"<<endl;
		for( unsigned int k = 0; k<mat.mk; k++ ) {
			cin >> mat.data[i][k];
		}
	}
...
//Funkcje pomocnicze
//ustawia daną kolumnę przy pomocy tablicy double, może być więcej elementów, nie mniej
...
Matrix operator + (const Matrix &mat1, const Matrix &mat2) {
	if( mat1.mw != mat2.mw || mat1.mk != mat2.mk ){
		cout << "Niezgodne wymiary! Zwracam pierwszą macierz." <<endl;
		return mat1;
	} 
	Matrix mat( mat1 );
	mat+=mat2;
...
//Interfejs użytkownika dla właściwej funkcji determinant
double Matrix::determinant() {
	/* Można również użyć funkcji minor 
	 return minor( -1, -1 );
	 */

	//sprawdzanie rozmiarów
	if( mw != mk ){
		cout << "Macierz powinna być kwadratowa!" << endl;
		return 0;
	}
	vector<unsigned int> vec; //wektor przechowujący wykreślone kolumny
	return determinant( 0, vec );
}

//Może zostać sukcesywnie zastąpione przez minor( -1, -1 );
double Matrix::determinant( unsigned int l, vector<unsigned int> vec ) {

	if( l == (mw-1) ) {
		//doszlimy do konca i zwracamy szukaną wartość
	...
	for( unsigned int i = 0; i<mw; i++ ) {
		...
		vec2.push_back( i );
		sort(vec2.begin(), vec2.end()); //sortowanie w celu uproszczenia wyszukiwania zakazanych kolumn w sekcji wyżej
...
//tak technicznie to jest to zmodyfikowany wyznacznik, który pozwala na wykreślenie danej kolumny i wiersza
//może również służyć do obliczania wyznacznika!!! wystarczy skorzystać z funkcji minor( -1, -1 );
double Matrix::minor( unsigned int l, vector<unsigned int> vec, unsigned int vec3 ) {
	
	//doszlimy do momentu gdy musimy zwrócić wartość
	if( ( l == (mw-2) && vec3 == mw-2 )  //wykreslamy przedostatni wiersz i przedostatnia kolumne
		...
		if( l == vec3 ) {
			l++; //jeśli jesteśmy w wykreślonym wierszu to pomijamy go
		}

		
		for( unsigned int m = 0; m < vec.size(); m++ ) {
			//szukamy kolumny, która nie została wykreślona, dlatego sortujemy vec po dodaniu wartości (niżej)
			if( m != vec[m] ) {
				return data[l][m];
			}
		}
		//gdy wykonujemy ten blok to oznacza, że zwracamy element leżący w ostatniej kolumnie
		
		unsigned int y = l;
		//przypadek gdy wykreślamy ostatni wiersz, to przesuwamy kolumnę która zawiera zwracany element w prawo.
		if( vec3 == mw-1 )
			y++;
	...
	//jeśli jesteśmy w wykreślonym wierszu to pomijamy go
	if( vec3 == l ) l++;

	double suma = 0; //zwracana wartość
	...
	for( unsigned int i = 0; i<mw; i++ ) {
		...
		suma += z*(data[l][i])*(minor( l+1, vec2, vec3 )); //dodawanie kolejnego składnika do sumy
